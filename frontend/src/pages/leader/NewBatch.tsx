      ...
      <Card title="新建活动批次">
        {loadError ? (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            <p style={{ color: '#ff4d4f', marginBottom: 16 }}>加载失败，请检查网络后重试</p>
          ...
          >
            <Form.Item name="batch_name" label="批次名称" rules={[{ required: true }]}>
              <Input placeholder="请输入批次名称" disabled={loadError} />
            </Form.Item>
            ...
            <Form.Item name="route_id" label="活动路线" rules={[{ required: true }]}>
              <Select
                ...
                loading={riskLoading}
                disabled={loadError || routes.length === 0}
              >
                {routes.map((r: any) => (
            ...
            <Form.Item
              ...
              rules={[
                ...
                {
                  validator: (_, value) => {
                  if (selectedRoute?.max_people && value > selectedRoute.max_people) {
                    return Promise.reject(new Error(`人数不能超过路线最大限制${selectedRoute.max_people}人`));
                  }
            ...
            <Form.Item name="activity_date" label="活动日期" rules={[{ required: true }]}>
              <DatePicker style={{ width: '100%' }} placeholder="请选择活动日期" disabledDate={(current) => current && current < dayjs().startOf('day')} disabled={loadError} />
            </Form.Item>

            <Form.Item label="活动时间">
              <Space>
                <Form.Item name="start_time" noStyle>
                  <TimePicker placeholder="开始时间" format="HH:mm" disabled={loadError} />
                </Form.Item>
                <span> - </span>
                <Form.Item name="end_time" noStyle>
                  <TimePicker placeholder="结束时间" format="HH:mm" disabled={loadError} />
                </Form.Item>
              </Space>
            </Form.Item>

            <Form.Item name="notes" label="备注">
              <TextArea rows={3} placeholder="请输入备注信息" disabled={loadError} />
            </Form.Item>

            <Form.Item>
              <Space>
                <Button type="primary" htmlType="submit" loading={loading} disabled={loadError}>
                  创建批次
                </Button>
                <Button onClick={() => navigate('/leader/batches')} disabled={loadError}>取消</Button>
              </Space>
            </Form.Item>
          </Form>
        )}
