        ...
        {loadError ? (
          ...
          >
            <Form.Item name="segment_id" label="选择路段" rules={[{ required: true }]}>
              <Select placeholder="请选择巡看路段" disabled={loading || segments.length === 0}>
                {segments.map((s) => (
            ...
            <Form.Item name="obstruction_status" label="遮挡状态">
              <Select placeholder="请选择遮挡状态" disabled={loadError}>
            ...
            <Form.Item name="obstruction_details" label="遮挡详情">
              <TextArea rows={3} placeholder="请描述遮挡情况" disabled={loadError} />
            </Form.Item>

            <Divider orientation="left">湿滑情况</Divider>
            <Form.Item name="slippery_warning" label="湿滑风险">
              <Select placeholder="请选择湿滑风险" disabled={loadError}>
            ...
            <Form.Item name="slippery_details" label="湿滑详情">
              <TextArea rows={3} placeholder="请描述湿滑情况" disabled={loadError} />
            </Form.Item>

            <Divider orientation="left">看台状态</Divider>
            <Form.Item name="bleachers_status" label="看台状态">
              <Select placeholder="请选择看台状态" disabled={loadError}>
                <Select.Option value="正常">正常</Select.Option>
            ...
            <Form.Item name="bleachers_details" label="看台详情">
              <TextArea rows={3} placeholder="请描述看台情况" disabled={loadError} />
            </Form.Item>

            <Divider orientation="left">总体评估</Divider>
            <Form.Item name="overall_status" label="总体状态" rules={[{ required: true }]}>
              <Select placeholder="请选择总体状态" disabled={loadError}>
            ...
            <Form.Item name="suggestions" label="处理建议">
              <TextArea rows={4} placeholder="请输入处理建议" disabled={loadError} />
            </Form.Item>
            ...
            <Form.Item>
              <Space>
                ...
                <Button onClick={() => navigate('/inspector/records')} disabled={loadError}>
                  取消
                </Button>
              </Space>
            </Form.Item>
