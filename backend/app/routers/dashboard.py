...
def get_overview(
    ...
    if end_date:
        batch_query = batch_query.filter(ActivityBatch.activity_date <= end_date + ' 23:59:59')
    ...
    if end_date:
        inspection_query = inspection_query.filter(InspectionRecord.inspection_date <= end_date + ' 23:59:59')
...
def get_anomaly_distribution(
    ...
    if end_date:
        obstruction_query = obstruction_query.filter(InspectionRecord.inspection_date <= end_date + ' 23:59:59')
    ...
    if end_date:
        slippery_query = slippery_query.filter(InspectionRecord.inspection_date <= end_date + ' 23:59:59')
    ...
    if end_date:
        bleachers_query = bleachers_query.filter(InspectionRecord.inspection_date <= end_date + ' 23:59:59')
...
def get_risk_overlap(
    ...
    if end_date:
        ...
        batch_query = batch_query.filter(func.date(ActivityBatch.activity_date) == today.date())
